...
def pkls_to_results_csvs(stage='dipsearch', inj=True):
    ...
    # These files are all regenerated by this routine.
    ...
    for lcdname in lcdnames:
        ...
        if loadfailed:
            continue
        # Write results tables. (Control flow logic: this is automatically
        # done for any run. So we needed to delete the table before ANY
        # run, and it'll reconstruct the table based on everything in the
        # saved pickles.)
        irra.write_injrecov_result(lcd, allq, stage=stage)
...
if __name__ == '__main__':
    ...
    ## If you need to process all the pickles to csv to write injrecov results.
    pkls_to_results_csvs()

    #TODO: make the above options arguments via parser
    pass
