...
def _residual_plots(dicts, titleinfo):
    ...
    dtitles= [
        ...
        int(dicts[0]['fourierorder']), dicts[0]['fitchisq'], dicts[0]['fitredchisq']),
        'univariate spline. knots: {:d}\n$\chi^2$: {:.3g}, red$\chi^2$: {:.3g}'.format(
        int(dicts[1]['nknots']), dicts[1]['fitchisq'], dicts[1]['fitredchisq']),
        'savitzky-golay. windowlen: {:d}\npolydeg: {:d}, $\chi^2$: {:.3g}, red$\chi^2$: {:.3g}'.format(
        dicts[2]['windowlength'], dicts[2]['polydeg'],dicts[2]['fitchisq'],dicts[2]['fitredchisq']),
        'savitzky-golay. windowlen: {:d}\npolydeg: {:d}, $\chi^2$: {:.3g}, red$\chi^2$: {:.3g}'.format(
        dicts[3]['windowlength'], dicts[3]['polydeg'],dicts[3]['fitchisq'],dicts[3]['fitredchisq']),
        'savitzky-golay. windowlen: {:d}\npolydeg: {:d}, $\chi^2$: {:.3g}, red$\chi^2$: {:.3g}'.format(
        dicts[4]['windowlength'], dicts[4]['polydeg'],dicts[4]['fitchisq'],dicts[4]['fitredchisq'])
        ]
    ...
    for ix, thisdict in enumerate(dicts):
        ...
        axs[0,ix].errorbar(thisdict['phase'],thisdict['mags'], fmt='ko',
                yerr=thisdict['errs'], markersize=1., capsize=0, alpha=0.1)
        axs[0,ix].plot(thisdict['phase'],thisdict['fitmags'], 'r-', linewidth=2.0)
        axs[0,ix].set(ylabel='normalized flux')
        axs[0,ix].set_title(dtitles[ix], fontsize=11)

        thisresidual = thisdict['mags']-thisdict['fitmags']
        axs[1,ix].errorbar(thisdict['phase'], thisresidual,
                fmt='ko',yerr=thisdict['errs'],markersize=2., capsize=0,
                alpha=0.05)
        ...
        dicts[ix]['residual'] = thisresidual
...
def get_residual_flux_vs_time(dicts, rescaletomedian=True):
    ...
    for ix, thisdict in enumerate(dicts):
        ...
        ptimes = thisdict['times']
        pmags = thisdict['mags']
        presiduals = thisdict['residual']
        perrs = thisdict['errs']
        phase = thisdict['phase']
        ...
        dicts[ix]['rtimes'] = rtimes
        dicts[ix]['rphase'] = rphase
        dicts[ix]['rmags'] = rmags
        dicts[ix]['rerrs'] = rerrs
...
def get_windowed_goodness_of_fit(dicts, titleinfo, N=20, datatype='keplerLC'):
    ...
    for ix, thisdict in enumerate(dicts):
        ...
        rtimes = thisdict['rtimes']
        rphase = thisdict['rphase']
        rmags = thisdict['rmags']
        rerrs = thisdict['rerrs']
...
def _visualize_worst_fits(dicts, titleinfo, N=10):
    ...
    axt_spl.errorbar(spldict['rtimes'], spldict['rmags'], fmt='ko',
            yerr=spldict['rerrs'], markersize=2., capsize=0, alpha=0.8,
            zorder=1)
    axt_sg.errorbar(sgdict['rtimes'], sgdict['rmags'], fmt='ko',
            yerr=sgdict['rerrs'], markersize=2., capsize=0, alpha=0.8,
            zorder=1)
    ...
    for ix in range(ncols*2):
        wintimes = spldict['rtimes'][windows[ix][0]]
        ...
        winmags = spldict['rmags'][windows[ix][0]]
        winerrs = spldict['rerrs'][windows[ix][0]]
    ...
    for ix in range(ncols*2):
        wintimes = sgdict['rtimes'][windows[ix][0]]
        wmax, wmin = max(wintimes), min(wintimes)
        ...
        # plot residual vs time in window on lower panel subplot
        winmags = sgdict['rmags'][windows[ix][0]]
        winerrs = sgdict['rerrs'][windows[ix][0]]
    ...
    #FIXME: get rid of all the above crap / fine-tune figure
...
def fit_test():
    ...
    for ix, ebpath in enumerate(ebpaths[:100]): #TODO run on everything
        ...
        or not os.path.exists(
            ...
            # TODO: test `get_windowed_goodness_of_fit`
